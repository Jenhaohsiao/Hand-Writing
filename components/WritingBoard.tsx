...
const WritingBoard: React.FC<WritingBoardProps> = ({ character, role, speed, size = 400, onComplete }) => {
  ...
  const initWriter = useCallback(() => {
    ...
    const options = {
      ...
      height: size,
      padding: size * 0.05,
      strokeAnimationSpeed: speed * 1.5,
    ...
    if (role === 'viewer') {
      ...
      const loop = () => {
        if (writerRef.current) {
          writerRef.current.animateCharacter({ onComplete: () => setTimeout(loop, 1500) });
        }
      };
      loop();
    } else {
    ...
    <div className="relative flex flex-col items-center bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none opacity-[0.03]">
        <svg width="100%" height="100%" viewBox="0 0 100 100">
          ...
          <line x1="0" y1="0" x2="100" y2="100" stroke="black" strokeWidth="0.5" strokeDasharray="2" />
          <line x1="100" y1="0" x2="0" y2="100" stroke="black" strokeWidth="0.5" strokeDasharray="2" />
        </svg>
      </div>
